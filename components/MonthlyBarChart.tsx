...
export default function MonthlyBarChart({ monthly, variant = "default", selectedMonths, style }: MonthlyBarChartProps) {
  ...
  if (isMini) {
    return (
      <View style={[tw`flex-row items-end`, { height: barHeight, gap: 2 }, style]}>
        {monthly.map((value, month) => {
          ...
          return (
            <View key={month} style={tw`flex-1 justify-end h-full`}>
              <View
                style={[
                  tw.style("w-full rounded-[1px]", isHighlighted(month) ? "bg-emerald-600" : "bg-gray-300"),
          ...
          return (
            ...
            >
              ...
              <Text
                style={tw.style(
                  "text-[10px] mt-1.5",
                  highlighted && !allMonths ? "text-emerald-700 font-bold" : "text-gray-500 font-medium"
                )}
              >
